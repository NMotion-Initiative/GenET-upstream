...
while IFS= read -r variable_name; do
  case "${variable_name}" in
    NODE_RANK|GENET_NODE_ID|GENET_CONTAINER_NAME|GENET_CONTAINER_DETACH|GENET_CONTAINER_RM)
      ;;
    NNODES|NPROC_PER_NODE|MASTER_ADDR|MASTER_PORT|PREFLIGHT_PORT|WAN_VAE_PATH|BASE_CHECKPOINT_PATH|DATASET_PATH|OMP_NUM_THREADS|HF_HOME|HF_HUB_CACHE|HF_HUB_OFFLINE|TRANSFORMERS_OFFLINE|GENET_*|NCCL_*|TORCH_NCCL_*|GLOO_SOCKET_IFNAME|WANDB_*|HTTP_PROXY|HTTPS_PROXY|http_proxy|https_proxy|NO_PROXY|no_proxy|ALL_PROXY|all_proxy)
